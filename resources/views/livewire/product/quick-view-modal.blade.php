...
<div
    ...
    x-show="show"
    x-on:keydown.escape.window="show = false; @this.call('closeModal')" {{-- Cierra con Escape --}}
    x-on:click.away="show = false; @this.call('closeModal')" {{-- Cierra al hacer clic fuera (opcional, aplica al contenedor del modal, no al overlay) --}}
    ...
    class="modal fixed inset-0 z-50 overflow-y-auto flex items-center justify-center" {{-- Clases de Tailwind/Alpine para posicionar y ocultar/mostrar --}}
    style="display: none;" {{-- Alpine gestionará la visibilidad --}}
    aria-labelledby="modal-title"
    role="dialog"
    aria-modal="true"
>
    {{-- Overlay oscuro --}}
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" x-show="show" x-on:click="show = false; @this.call('closeModal')"></div>

    {{-- Contenedor del Modal --}}
    <div class="modal-dialog quickview__main--wrapper relative bg-white rounded-lg shadow-xl overflow-hidden transform transition-all sm:max-w-4xl sm:w-full mx-auto" {{-- Ajusta max-w-* según tu diseño --}}>

        {{-- Solo renderiza el contenido si hay un producto --}}
        @if($product)
            <header class="modal-header quickview__header">
                {{-- Botón de cierre llama al método Livewire Y actualiza Alpine --}}
                <button wire:click="closeModal" x-on:click="show = false" class="close-modal quickview__close--btn" aria-label="close modal">✕</button>
            </header>
            <div class="quickview__inner">
                <div class="row row-cols-lg-2 row-cols-md-2">
                    <div class="col">
                        {{-- MEDIA DEL PRODUCTO (Imágenes, Swiper) --}}
                        <div class="product__details--media mb-4 md:mb-0">
                            {{-- Imagen Principal --}}
                            <div class="product__media--preview mb-4">
                                 <img class="w-full h-auto object-cover rounded" src="{{ $product->getFirstMediaUrl('product_images', 'preview') ?: 'placeholder.jpg' }}" alt="{{ $product->name }}">
                                 {{-- Aquí podrías necesitar Swiper para la imagen principal si es un slider --}}
                            </div>

                            {{-- Miniaturas (Si usas Swiper para thumbs) --}}
                            ...
                            <div class="product__media--nav swiper">
                                <div class="swiper-wrapper">
                                    ...
                                    <div class="swiper-slide">
                                        <div class="product__media--nav__items">
                                             <img class="product__media--nav__items--img cursor-pointer" src="{{ $media->getUrl('thumbnail') ?: 'placeholder.jpg' }}" alt="thumbnail {{ $loop->iteration }}">
                                         </div>
                                     </div>
                                    @endforeach
                                </div>
                                {{-- Añade botones de navegación de Swiper si los usas --}}
                                <div class="swiper__nav--btn swiper-button-next"></div>
                                <div class="swiper__nav--btn swiper-button-prev"></div>
                            </div>
                            @endif
                    ...
                    <div class="col">
                         {{-- INFO DEL PRODUCTO --}}
                        <div class="quickview__info p-4">
                             <h2 class="product__details--info__title text-2xl font-semibold mb-3" id="modal-title">{{ $product->name }}</h2>
                            <div class="product__details--info__price mb-3">
                                <span class="current__price text-xl font-bold text-red-600">S/.{{ number_format($product->price, 2) }}</span>
                                {{-- <span class="old__price text-gray-500 line-through ml-2">S/.{{ number_format($product->old_price, 2) }}</span> --}}
                            </div>
                             {{-- Ratings, descripción, variantes, etc. --}}
                             <p class="product__details--info__desc text-gray-600 mb-4">{{ $product->description ?: 'Descripción no disponible.' }}</p>

                            {{-- Aquí irían las variantes, cantidad, botón AddToCart --}}
                            {{-- <button wire:click="addToCart({{ $product->id }})" class="primary__btn ...">Add To Cart</button> --}}

                            {{-- Resto de la info: wishlist, share, etc. --}}
                        </div>
                    </div>
                </div>
            </div>
        @else
            {{-- Estado de carga o vacío (opcional) --}}
