            ...
            <ul>
                ...
                @empty
                    <li class="text-center py-4">
                        <p>{{ __('No hay productos en el carrito') }}</p>
                    </li>
                @endforelse
            </ul>
            <div class="cart-qty">
                <h4>{{ __('Cantidad') }}: <span>{{ $qty }}</span></h4>
            </div>
            <div class="cart-total">
                <h4>{{ __('Subtotal') }}: <span>${{ $subtotal }}</span></h4>
            </div>
            <div class="cart-checkout-btn">
                <a class="btn-hover cart-btn-style" href="{{ route('cart') }}">{{ __('Ver carrito') }}</a>
                <a class="no-mrg btn-hover cart-btn-style" href="{{ route('checkout.index') }}">{{ __('Ir a pagar') }}</a>
            </div>
        </div>
    </div>
</div>